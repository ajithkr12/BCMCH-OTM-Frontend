...
const BookingRegistrationForm = (props) => {
    ...
    const FetchOtherDepartmentSurgeons = async (selectedDepartments) =>{
        const _data = await GetOtherDepartmentSurgeons(selectedDepartments, 1, 10000);
                ...
                <Grid item md ={3} style={useStyles.root}>
                    <Autocomplete
                        ...
                        onChange={(event, newValue) => {
                            ...
                            console.log("JSON.stringify : ", JSON.stringify(newValue, null, ' '));
                        }}

                        inputValue={inputValueSurgery||""}
                        onInputChange={ async (event, newInputValue) => {
                            var _data_fetched = await GetSurgeryList(newInputValue, 1, 50);
                        ...
                        >
                            {
                                Masters.anaesthetistList.map((data) => {
                                    return (
                                        <MenuItem key={data.employeeId} value={data.employeeId}>{data.firstName+" "+data.middleName+" "+data.lastName}</MenuItem>
                                    )})
                            }
                        </Select>
                ...
                <Grid item md ={3} style={useStyles.root}>
                    <Controller
                        ...
                        render={({ field: { value, onChange } }) => (
                            <FormControl fullWidth>
                                <InputLabel id="other-department">Other Departments</InputLabel>
                                <Select
                                    {...register}
                                    ...
                                    onChange={async(e) => {
                                        ...
                                        console.log("setDepartmentsSelected : ", departmentSelected)
